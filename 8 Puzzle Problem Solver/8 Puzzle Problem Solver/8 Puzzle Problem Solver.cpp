...
bool solved = false;
bool visit(vector<vector<int>> a){ return visited[a]; }
	...
	for (int i = 0; i<3; i++){
		for (int j = 0; j<3; j++){
			if (a[i][j] != 0){
				for (int k = 0; k<3; k++){
					for (int l = 0; l<3; l++){
						if (a[i][j] == goal[k][l])dist += abs(i - k) + abs(j - l);
					}
				}
			}
		}
...
bool checkgrid(vector<vector<int> > a){
	for (int i = 0; i<3; i++){
		for (int j = 0; j<3; j++){
			if (a[i][j] != goal[i][j])
				return false;
...
void print_path(vector<vector<int>> s){
	solved = true;
	if (parent.count(s))print_path(parent[s]);
	for (int i = 0; i<3; i++){
		for (int j = 0; j<3; j++){
			cout << s[i][j] << " ";
		}
