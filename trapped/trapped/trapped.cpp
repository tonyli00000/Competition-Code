...
int solve(int L, int r)
{
	...
	if (power > bales[L].size && power > bales[r].size)return min(solve(L - 1, r), solve(L, r + 1));
	else if (power > bales[L].size)return min(solve(L - 1, r), power - bales[L].size);
	else if (power > bales[r].size)return min(power - bales[r].size, solve(L, r + 1));
	else return 0;
}
